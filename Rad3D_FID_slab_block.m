...
addpath(genpath('.'));
dw = 10e-6;
rfdt = 10e-6; %change this back if this is not working to v12 seq
...
Nx=256; Nz =256;% Will work with lesser for now - change it back to 64
sliceThickness=128e-3;%was 10e-3 before
TR = 20e-3;
TE = 0.5e-3;%minimum 
dx = fov/Nx;
...
[rf,gz] = mr.makeBlockPulse(pi/30,'Duration',dw, 'SliceThickness',sliceThickness ,'system', system); % dummy gz is needed here for calculate spoil and delay time, won't be added to the sequence 
% [rf] = mr.makeBlockPulse(pi/30,'Duration',dw, 'system', system);% This is the new addition; may break sequence :)
%% Gradients
...
delayTR=TR - mr.calcDuration(rf) ... %Tricky here, not consistent
    - mr.calcDuration(gx) - mr.calcDuration(gzSpoil);
...
fname = ['Rad3D_FID_v13', num2str(radp.Ns),'_',num2str(Nx),'_st',num2str(TE),'.seq'];
seq.write(fname)
