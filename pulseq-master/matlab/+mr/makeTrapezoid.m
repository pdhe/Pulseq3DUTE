...
if opt.flatTime>0
    if ~isempty(opt.amplitude) %amplitude given
        amplitude = opt.amplitude;
    else %amplitude not given
    ...
    fallTime = riseTime;
    flatTime = opt.flatTime; %defines the flat time
elseif opt.duration>0
    if ~isempty(opt.amplitude)
        amplitude = opt.amplitude; % defines the amplitude if it is  given
    else
        if isempty(riseTime)
            dC = 1/abs(2*maxSlew) + 1/abs(2*maxSlew);
            possible = opt.duration^2 > 4*abs(opt.area)*dC;
            amplitude = ( opt.duration - sqrt(opt.duration^2 - 4*abs(opt.area)*dC) )/(2*dC); % defines amplitude if rise time is not given
        else
            amplitude = opt.area/(opt.duration-riseTime); %defines amplitude if rise time is given
